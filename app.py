...
def load_image(uploaded_file, default_path):
    if uploaded_file is not None:
        return Image.open(uploaded_file).convert("RGB")
    elif os.path.exists(default_path):
        return Image.open(default_path)
    else:
        st.error("Image par défaut introuvable.")
        return None

uploaded_file = st.file_uploader("Choisissez une image", type=["jpg", "jpeg", "png"])
img = load_image(uploaded_file, default_image_path)
...
tabs = st.tabs([
    ...
    "Contours avancés (Canny)"
])

# Fonction pour afficher les canaux de couleur
...
# Fonction pour niveaux de gris avec téléchargement
def display_grayscale(img_gray, gray_scale):
    ...
    with col_gray_image:
        ...
        buf = io.BytesIO()
        img_quantified.save(buf, format="PNG")
        st.download_button(
            label="Télécharger l'image en niveaux de gris",
            data=buf.getvalue(),
            file_name=f"grayscale_{gray_scale}.png",
            mime="image/png"
        )
    with col_hist:
        st.pyplot(processing.histogram(img_quantified, gray_scale))

# Fonction pour afficher les flous
def display_blur(img):
    st.subheader("Flouter une image")
    ...
    with col_slider_blur1:
        ...
        st.write("Le flou Gaussien réduit les détails de l'image en utilisant une fonction mathématique 'en cloche' (la courbe de Gauss). Contrairement à un flou classique, il accorde plus d'importance aux pixels centraux qu'aux pixels éloignés, ce qui donne un rendu très naturel et fluide.")
    with col_slider_blur2:
        ...
        st.write("Ce filtre remplace la couleur de chaque pixel par la moyenne arithmétique de tous les pixels voisins. Plus le rayon est grand, plus l'image semble 'diluée'. C'est un filtre simple et rapide, mais il peut donner un aspect un peu plus rigide et géométrique que le flou Gaussien.")
    with col_slider_blur3:
        ...
        st.write("Le flou médian ne fait pas de calcul de moyenne : il trie les pixels voisins et choisit la valeur centrale (la médiane). Son grand pouvoir est de supprimer le bruit numérique (comme les petits points blancs ou noirs) tout en préservant la netteté des bords. C'est le filtre préféré pour nettoyer des images dégradées sans les rendre totalement floues.")

# Fonction pour égalisation d'histogramme
def display_histogram_equalization(img):
    st.subheader("Égalisation d'histogramme (en niveaux de gris)")
    st.write("Ce traitement étire les contrastes. Si une image est trop sombre ou trop grise, il redistribue les intensités sur toute la plage (0-255).")
    st.write("EXEMPLE :")
...
def display_binarization(img_gray_hist):
    ...
    with col_bin_otsu:
        ...
        st.write("La méthode d'Otsu est dite 'intelligente'. Elle analyse l'histogramme de l'image pour trouver mathématiquement le seuil qui sépare le mieux le premier plan de l'arrière-plan.")

# Fonction pour opérations morphologiques
def display_morphological_operations(img_bin_otsu):
    st.subheader("Opérations morphologiques")
    st.write("Ces procédés sont basés sur la forme de l'image et s'appliquent généralement sur des images binaires (noir et blanc). Ils utilisent deux éléments : une image et un noyau (kernel) qui définit la zone d'influence du filtre.")
    kernel_size = st.slider("Choisissez la taille du noyau", min_value=1, max_value=15, value=2)
    ...
    with col_erode:
        ...
        st.image(img_erode, caption=f"Érosion avec un noyau de ({kernel_size} x {kernel_size}) effectuée {iterations} fois")
        st.write("L'érosion 'grignote' les bords des objets blancs. Si un pixel blanc a au moins un voisin noir, il devient noir. Utile pour supprimer les petits bruits blancs isolés (parasites) ou détacher deux objets qui se touchent à peine.")
    with col_dilate:
        ...
        st.write("C'est l'inverse de l'érosion. Elle ajoute des pixels blancs sur les bords des objets. Si un pixel noir a un voisin blanc, il devient blanc. Utile pour combler des petits trous noirs à l'intérieur d'un objet ou relier des parties brisées d'une même forme.")
    with col_open:
...
# Fonction pour détection de contours avancée (Canny)
def display_canny_edges(img_gray):
    st.subheader("Détection de contours avec Canny")
    st.write("L'algorithme de Canny détecte les bords en utilisant des seuils pour minimiser le bruit et les faux positifs.")
    min_thresh = st.slider("Seuil minimum", 0, 255, 100)
    max_thresh = st.slider("Seuil maximum", 0, 255, 200)
    edges = processing.canny_edge_detection(img_gray, min_thresh, max_thresh)
    edges.thumbnail((500, 500))  # Redimensionner pour l'affichage
    _, col, _ = st.columns([1, 2, 1])  # Centrer l'image dans une colonne centrale
    with col:
        st.image(edges, caption=f"Contours détectés (Canny, seuils {min_thresh}-{max_thresh})", use_container_width=True)

# Fonction pour détection de contours
def display_contour_detection():
    st.subheader("Détection de contours")
    st.write("Pour détecter les contours sur une image, elle doit être pré-traitée avec certaines méthodes décrites précédemment.")
    examples_images = {
    ...
    if choice == "Charger mon image":
        ...
        if os.path.exists(path):
            ...
            st.error(f"Image {path} introuvable.")
            return
    ...
    tabs_inner = st.tabs(["1. Gris & Flou", "2. Binarisation", "3. Morphologie", "4. Résultat Final"])
    with tabs_inner[0]:
...
# Appeler les fonctions dans les onglets
with tabs[0]:
    display_color_channels(img_display)
...
with tabs[1]:
    ...
    display_grayscale(img_gray, gray_scale)

with tabs[2]:
    display_blur(img_display)

with tabs[3]:
    display_histogram_equalization(img)

with tabs[4]:
    img_gray_hist = img.convert("L").quantize(colors=255)
    display_binarization(img_gray_hist)

with tabs[5]:
    _, img_bin_otsu = processing.otsu(img_gray_hist)
    display_morphological_operations(img_bin_otsu)

with tabs[6]:
    display_contour_detection()

with tabs[7]:
    img_gray_for_canny = img.convert("L")
    display_canny_edges(img_gray_for_canny)
